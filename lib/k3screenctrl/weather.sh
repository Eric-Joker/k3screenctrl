...
weather_info=$(cat /tmp/k3screenctrl/k3-weather.json 2>/dev/null)
...
if [ "$update_weather" = "1" ]; then
	rm -rf /tmp/k3screenctrl/k3-weather.json
	wget "http://api.seniverse.com/v3/weather/now.json?key=$key&location=$city&language=zh-Hans&unit=c" -T 3 -O /tmp/k3screenctrl/k3-weather.json 2>/dev/null
fi
